...
public record UpdateUserRequest(String name, String password, String document, String phone, Address address) {

    public User update(User user) {
        user.setModifiedAt(LocalDateTime.now());
        if (this.name != null) user.setName(this.name);
        if (this.password != null) user.setPassword(this.password);
        if (this.document != null) user.setDocument(this.document);
